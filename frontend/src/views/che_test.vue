...
const loadAllElements = () => {
  fetch("http://127.0.0.1:8000/api/v1/chemical/element/all")
      ...
      if (windowSize.value.clientWidth > 1970) {
        ...
        for (let elem of allElements) {
          ...
          if (elementDom) {
            // 修改元素名称（找到 .element-name 类的子元素）
            // const newDivHtml = `${elem.symbol}`;
            const newDivHtml = `<div class="element-symbol-small">${elem.atomic}</div>`;
            elementDom.innerHTML += newDivHtml;
            elementDom.classList.add(elem.type);
          }
        }
...
const setAllRatioItems = () => {
  updateWindowSize();
  console.log("窗口宽度:", windowSize.value.width);

  const items = Array.from(document.querySelectorAll(".grid-element"));
  console.log("匹配到的元素数量：", items.length);
  items.forEach((item) => {
    if (item) {
      console.log(item);
...
<template>
  <h1 style="text-align: center; margin: 5px">{{ title }}</h1>
